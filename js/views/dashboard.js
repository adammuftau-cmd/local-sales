...
function drawTrendChart(state) {
  ...
  // Chart.js loads from a CDN. If there's no connection the first time the app
  // runs (before it's been cached), skip the chart instead of crashing the
  ...
  if (typeof Chart === 'undefined') {
    ...
    if (wrap && !wrap.querySelector('.chart-unavailable')) {
      ...
      note.style.cssText = 'color:var(--ink-soft);font-size:0.85rem;padding:20px 0;text-align:center;';
      note.textContent = 'Chart unavailable offline — connect once so it can be cached, then it\u2019ll work offline too.';
